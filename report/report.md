...
The transition table is another dictionary, where the key is a tuple (state, letter) and the value another tuple (next state, new letter, direction). This dictionary is the used by the transition function to look up transitions.
...
Parsing code reads the definition file line by line and tries to parse these lines using the Python `split` method and assigning the result to multiple variables. It checks for any errors while doing that and throws a `ParseError` if the definition file has invalid format. It includes multiple checks, for example:
...
1. Assume that the list is already sorted and go through them from left to right.
2. If an out-of-order element is found, remember it and replace it by the expected elemenent (e.g. $11223341 \rightarrow 11223344$).
...
I have tried to test all edge cases for each solution. The tested inputs are commented in the test files.
